...
def generate_optimized_charset_from_frequency(freq_table, include_zero_freq=False):
   '''
   TODO: formalize help blurb
   Given a character frequency table such as those returned by generate_frequency_table(),
   return ``bytes`` with only single characters sorted by frequency of occurance descending
   '''
...
def generate_frequency_table(text,charset):
   ...
   text_len = len(text)
   if text_len == 0:
      raise Exception("Text is zero-length.")
...
def string_to_long(instring):
   '''
   Take bytes and convert it to a number
   Since conversion to Python3, this function is confusingly named.
   It takes in ``bytes`` and returns ``int``. 

   ``bytes`` instring - Sample to convert
   '''
   return int(hexlify(instring),16)

def long_to_string(inlong):
   '''
   Take a long and convert it to a string
   Since conversion to Python3, this function is confusingly named.
   It takes in ``int`` and returns ``bytes``. 
   
   ``int`` inlong - Number to convert
   '''
