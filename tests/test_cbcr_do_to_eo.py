...
def my_decryption_oracle(ciphertext):
   ...
   try:
      plaintext = ca.pkcs7_padding_remove(plaintext,AES.block_size)
      return plaintext
   except ValueError:
      return plaintext
...
def my_padding_oracle(ciphertext):
   plaintext = cipher.decrypt(ciphertext)
   try:
      ca.pkcs7_padding_remove(plaintext,AES.block_size)
      return True
   except ValueError:
      return False

new_plaintext = b'I am the very model of a modern major-general.'
